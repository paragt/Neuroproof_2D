...
void StackPredict::agglomerate_rag_flat(double threshold, bool use_edge_weight, string output_path, string classifier_path)
{
    ...
    FILE* initial_fid_edge = fopen(wts_path.c_str(), "wt");
    FILE* initial_fid_wt = fopen(wts_path2.c_str(), "wt");
  
    fprintf(initial_fid_edge, "%u\n",count);
    ...
    for (Rag<Label>::edges_iterator iter = rag->edges_begin(); iter != rag->edges_end(); ++iter) {
        ...
        Label node1 = rag_node1->get_node_id(); 
	Label unode1 = unique_node_id[node1];
        Label node2 = rag_node2->get_node_id();
	Label unode2 = unique_node_id[node2];
    ...
    string wts_path3 = dirname+ "/"+ wts_fname + "_prob_beta"+ tmpstr + "_mi100_cplex.txt";	
...
// 	    (rag_node2->get_node_type()==2 && rag_node1->get_node_type()==1))	
// 	   continue;
	if(Stack::merge_mito)    
	    if ((rag_node1->get_node_type()==2) || (rag_node2->get_node_type()==2))	
	      continue;
...
void StackPredict::agglomerate_rag_link(double threshold, bool use_edge_weight, string output_path, string classifier_path)
{
...
//     Q->set_storage(&all_edges);	
    ...
    for(size_t ii=0; ii< all_edges.size(); ii++) {
	
//     while (!Q->is_empty()){
// 	QE tmpqe = Q->heap_extract_min();	


	QE tmpqe = all_edges[ii];	
        //RagEdge<Label>* rag_edge = tmpqe.get_val();
        Label node1 = tmpqe.get_val().first;
        Label node2 = tmpqe.get_val().second;
        RagEdge<Label>* rag_edge = rag->find_rag_edge(node1,node2);;
	...
	double prob = tmpqe.get_key();
        if (!rag_edge || !tmpqe.valid() || prob >0.3) {
            continue;
        }

	RagNode<Label>* rag_node1;
	RagNode<Label>* rag_node2;
	if (rag_edge->get_node1()->get_size() > rag_edge->get_node2()->get_size()){
	    rag_node1 = rag_edge->get_node1();
	    rag_node2 = rag_edge->get_node2();
	}
	else{
	    rag_node1 = rag_edge->get_node2();
	    rag_node2 = rag_edge->get_node1();
	}
        node1 = rag_node1->get_node_id(); 
        node2 = rag_node2->get_node_id(); 
...
// 	    error += ((assignment.find(node1)->second == assignment.find(node2)->second) ? 0 : 1);
        ...
        rag_merge_edge_link(*rag, rag_edge, rag_node1, all_edges, feature_mgr);
        watershed_to_body[node2] = node1;
        for (std::vector<Label>::iterator iter = merge_history[node2].begin(); iter != merge_history[node2].end(); ++iter) {
            watershed_to_body[*iter] = node1;
        }
